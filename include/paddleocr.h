...
namespace PaddleOCR {
    ...
    public:
        ...
        ocr(const std::vector<cv::Mat> &img_list,
            bool det = true, bool rec = true, bool cls = true) noexcept;

        std::vector<OCRPredictResult>
        ocr(const cv::Mat &img, bool det = true,
            bool rec = true, bool cls = true) noexcept;

        void reset_timer() noexcept;
        void benchmark_log(int img_num) const noexcept;
